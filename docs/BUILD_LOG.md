...
| C1 | Design system | 🔄 reopened — rebuilt from the design project | |
...
Waiting on the design files. `DesignSync` cannot authorise from this remote cloud session
(`/design-login` needs an interactive session on the user's own machine, which would not carry
over to a cloud container), and nothing has been seeded into the workspace. `Njiani Apps.dc.html`
and `support.js` are to be attached directly.
