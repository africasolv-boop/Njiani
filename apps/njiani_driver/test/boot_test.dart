...
void main() {
  testWidgets('boots and renders the shared boot screen', (tester) async {
    await tester.pumpWidget(const NjianiBootApp(app: NjianiApp.driver));
    ...
    expect(find.text(NjianiApp.driver.label), findsOneWidget);
  ...
  testWidgets('main() launches this app, not the other one', (tester) async {
    ...
    expect(find.text(NjianiApp.driver.label), findsOneWidget);
    expect(find.text(NjianiApp.values.firstWhere((a) => a != NjianiApp.driver).label),
        findsNothing);
  });
}
